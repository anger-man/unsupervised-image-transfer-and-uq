...
parser.add_option('--task', action="store", dest="task", default="dt4/")
...
def define_composite_model(gCtoD, dD1, dD2, inp_dim,lr, coord,alpha,beta,lam):
    ...
    circ2 = Lambda(l1)([outimg_lpf, 
                        MyFilter(coord)(outimg_ori),MyFilter(coord)(uncerori)])
    circ1 = Lambda(l1)([outimg_lpf, 
                        MyFilter(coord)(outimg_ori),uncerlpf])
...
# 
#%%
# try effect of uncertainty measure

# try:
#     gCtoD.set_weights(np.load('weightsUAPI.npy',allow_pickle=True))
# except:
#     pass;
...
# # np.save('uncer_samples.npy', samples)
# samples=np.load('uncer_samples.npy')
...
# X1 = scale_data(X1,stage_dims[STAGE])
# X2 = load_data(task,len(samples),0,evinp,evtar,domain='target')
# X2 = scale_data(X2,stage_dims[STAGE])
   
# # X1=np.expand_dims(np.expand_dims(imread('model_img.jpg'),0),-1)/127.5 -1 
# tmp = gCtoD.predict(X1, batch_size=2)
# preds_B = tmp[...,:1]
# uncer = tmp[...,1:]
...
#     preds_B = (preds_B+1)/2
#     X2 = (X2+1)/2
# if task in ['ixi/','rir/','kne/']:
#     preds_B = (preds_B+1)*127.5
#     X2 = (X2+1)*127.5
# if task in ['dt4/']:
#     preds_B*=5; X2*=5
 
# avg_pool = AveragePooling2D(pool_size=(2,2),strides=1, padding='same')
...
# #     plt.axis('off')
# #     ax.imshow(preds_B[i],cmap='Greys_r')
# # for i in range(5):
# #     ax=fig.add_subplot(6, 5, 2*5+1 + i)
# #     plt.axis('off')
# #     ax.imshow(X2[i],cmap='Greys_r')
    
# # for i in range(5):
# #     ax=fig.add_subplot(6, 5, 3*5+1 + i)
# #     plt.axis('off')
# #     ax.imshow(np.sqrt(np.square(preds_B-X2))[i],cmap='Reds')
# # for i in range(5):
# #     ax=fig.add_subplot(6, 5, 4*5+1 + i)
# #     plt.axis('off')
# #     ax.imshow(uncer[i],cmap='Reds',vmin=0,vmax=1)


# # if paired:
# #     dif1 = psnr(X2,preds_B)
# #     # if task == 'dt4/':
# #     #     dif1 = np.mean(np.abs(avg_pool(X2)-avg_pool(preds_B)))*5
# #     q = np.quantile(uncer,0.99,axis=(1,2,3))
# #     ubool=[]
# #     for i in range(X1.shape[0]):
# #         ubool.append((uncer[i]<q[i]))
# #     ubool = np.array(ubool)
# #     udif = psnr(X2[ubool],preds_B[ubool])
# #     if task == 'dt4/':
# #         dif1 = np.mean(np.abs(avg_pool(X2)-avg_pool(preds_B)))*5
# #         udif = np.mean(np.abs((avg_pool(X2)-avg_pool(preds_B))[ubool]))*5
...
# j=8
# fig, ax = plt.subplots(4,5,figsize=(12.5*2/3,3.9*2/3))
# inds = [1,2,4,8,16]
# for kk in range(4):
#     j = inds[kk]
#     im = ax[kk,0].imshow(X1[j].astype(np.uint8), cmap='Greys_r')
#     ax[kk,0].axis('off')
#     im = ax[kk,1].imshow(X2[j], cmap='Greys_r')
#     ax[kk,1].axis('off')
#     im = ax[kk,2].imshow(preds_B[j], cmap='Greys_r')
#     ax[kk,2].axis('off')
#     im = ax[kk,3].imshow(gaussian(np.abs(X2[j]-preds_B[j]),(1.5,1.5,1)), cmap='jet',vmax=2.5)
#     plt.colorbar(im,ax=ax[kk,3])
#     ax[kk,3].axis('off')
#     im = ax[kk,4].imshow(uncer[j], cmap='jet',vmax=.46)
#     plt.colorbar(im,ax=ax[kk,4])
#     ax[kk,4].axis('off')
#     plt.imsave('christian/%02dinput.jpg'%j,X1[j].astype(np.uint8))
#     cv2.imwrite('christian/%02dground_truth.jpg'%j,255*(X2[j,...,0]+5)/10)
#     cv2.imwrite('christian/%02dprediction.jpg'%j,255*(preds_B[j]+5)/10)
#     cv2.imwrite('christian/%02dabs_residuals.jpg'%j,np.abs(X2[j]-preds_B[j])/5*255)
...
# resscor = np.mean(np.abs(X2-preds_B),axis=(1,2,3))*5
# uncscor = np.mean(uncer,axis=(1,2,3))
# plt.plot(uncscor, resscor,'o')

# df = pd.DataFrame(columns=['x_coord','y_coord'])
# df['x_coord'] = pd.Series(uncscor)
# df['y_coord'] = pd.Series(resscor)
# df.to_csv('xy-coordinates.csv')
# np.argsort(np.mean(uncer,axis=(1,2,3)))
