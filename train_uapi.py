...
# # investigate impact of joint uncertainty estimation
...
#     X2 = apply_filter(X2,d0=32,hpf=False)
...
#     plt.axis('off')
#     ax.imshow(uncer[i],cmap='Reds',vmin=0,vmax=.7)

# # print(np.mean(np.abs(X2-preds_B)))
# # print(np.mean(np.square(X2-preds_B))**.5)
# # plt.plot(-np.sort(-np.reshape(X2[0],-1))); plt.plot(-np.sort(-np.reshape(preds_B[0],-1)));
# # blc1 = -np.sort(-np.reshape(X2,(len(X2),256**2)));
# # blc2 = -np.sort(-np.reshape(preds_B,(len(preds_B),256**2)));
# # print(np.mean(np.abs(blc1-blc2)))
...
#     if task == 'dt4/':
#         dif1 = np.mean(np.abs(X2-preds_B))*5
#         udif = np.mean(np.abs(X2-preds_B)[ubool])*5
